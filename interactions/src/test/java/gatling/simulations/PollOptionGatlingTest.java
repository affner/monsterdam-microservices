...
public class PollOptionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all pollOptions")
                    .get("/services/interactions/api/poll-options")
                ...
                .exec(
                    http("Create new pollOption")
                        .post("/services/interactions/api/poll-options")
                        .headers(headers_http_authenticated)
                ...
                .on(
                    exec(
                        http("Get created pollOption")
                            .get("/services/interactions${new_pollOption_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created pollOption")
                        .delete("/services/interactions${new_pollOption_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
