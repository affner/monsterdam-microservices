 ...
 */
public class ChatRoomGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all chatRooms")
                    .get("/services/interactions/api/chat-rooms")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new chatRoom")
                        .post("/services/interactions/api/chat-rooms")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"lastAction\": \"SAMPLE_TEXT\"" +
                                ", \"lastConnectionDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"muted\": null" +
                ...
                .repeat(5)
                .on(
                    exec(http("Get created chatRoom").get("/services/interactions${new_chatRoom_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(
                    http("Delete created chatRoom").delete("/services/interactions${new_chatRoom_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ChatRoom entity").exec(scn);
