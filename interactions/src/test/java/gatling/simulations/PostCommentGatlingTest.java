...
public class PostCommentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all postComments")
                    .get("/services/interactions/api/post-comments")
                ...
                .exec(
                    http("Create new postComment")
                        .post("/services/interactions/api/post-comments")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created postComment")
                            .get("/services/interactions${new_postComment_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created postComment")
                        .delete("/services/interactions${new_postComment_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
