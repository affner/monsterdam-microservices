...
/**
 * Performance test for the LikeMark entity.
 */
public class LikeMarkGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all likeMarks")
                    .get("/services/interactions/api/like-marks")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new likeMark")
                        .post("/services/interactions/api/like-marks")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_likeMark_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created likeMark").get("/services/interactions${new_likeMark_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(
                    http("Delete created likeMark").delete("/services/interactions${new_likeMark_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the LikeMark entity").exec(scn);
