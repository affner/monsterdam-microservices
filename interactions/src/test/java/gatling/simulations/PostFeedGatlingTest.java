...
/**
 * Performance test for the PostFeed entity.
 */
public class PostFeedGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all postFeeds")
                    .get("/services/interactions/api/post-feeds")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new postFeed")
                        .post("/services/interactions/api/post-feeds")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_postFeed_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created postFeed").get("/services/interactions${new_postFeed_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(
                    http("Delete created postFeed").delete("/services/interactions${new_postFeed_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PostFeed entity").exec(scn);
