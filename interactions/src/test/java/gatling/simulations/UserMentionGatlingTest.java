...
public class UserMentionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all userMentions")
                    .get("/services/interactions/api/user-mentions")
                ...
                .exec(
                    http("Create new userMention")
                        .post("/services/interactions/api/user-mentions")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created userMention")
                            .get("/services/interactions${new_userMention_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created userMention")
                        .delete("/services/interactions${new_userMention_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
