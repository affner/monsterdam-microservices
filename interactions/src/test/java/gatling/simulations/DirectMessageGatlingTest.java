...
public class DirectMessageGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all directMessages")
                    .get("/services/interactions/api/direct-messages")
                ...
                .exec(
                    http("Create new directMessage")
                        .post("/services/interactions/api/direct-messages")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created directMessage")
                            .get("/services/interactions${new_directMessage_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created directMessage")
                        .delete("/services/interactions${new_directMessage_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
