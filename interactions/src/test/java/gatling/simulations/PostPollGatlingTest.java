...
/**
 * Performance test for the PostPoll entity.
 */
public class PostPollGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all postPolls")
                    .get("/services/interactions/api/post-polls")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new postPoll")
                        .post("/services/interactions/api/post-polls")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_postPoll_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created postPoll").get("/services/interactions${new_postPoll_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(
                    http("Delete created postPoll").delete("/services/interactions${new_postPoll_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PostPoll entity").exec(scn);
