...
/**
 * Performance test for the PollVote entity.
 */
public class PollVoteGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all pollVotes")
                    .get("/services/interactions/api/poll-votes")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new pollVote")
                        .post("/services/interactions/api/poll-votes")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"createdDate\": \"2020-01-01T00:00:00.000Z\"" + ", \"votingUserId\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_pollVote_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created pollVote").get("/services/interactions${new_pollVote_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(
                    http("Delete created pollVote").delete("/services/interactions${new_pollVote_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PollVote entity").exec(scn);
