...
import { searchEntities, getEntities } from './user-ui-preferences.reducer';

export const UserUIPreferences = () => {
  ...
  return (
    <div>
      ...
      <div className="table-responsive">
        {userUIPreferencesList && userUIPreferencesList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
            ...
            <tbody>
              {userUIPreferencesList.map((userUIPreferences, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/user-ui-preferences/${userUIPreferences.id}`} color="link" size="sm">
                      {userUIPreferences.id}
                    </Button>
                  </td>
                  <td>{userUIPreferences.preferences}</td>
                  <td>
                    {userUIPreferences.createdDate ? (
                      <TextFormat type="date" value={userUIPreferences.createdDate} format={APP_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td>
                    {userUIPreferences.lastModifiedDate ? (
                      <TextFormat type="date" value={userUIPreferences.lastModifiedDate} format={APP_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td>{userUIPreferences.createdBy}</td>
                  <td>{userUIPreferences.lastModifiedBy}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button
                        tag={Link}
                        to={`/user-ui-preferences/${userUIPreferences.id}`}
                        color="info"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`/user-ui-preferences/${userUIPreferences.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/user-ui-preferences/${userUIPreferences.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
      ...
      {totalItems ? (
        <div className={userUIPreferencesList && userUIPreferencesList.length > 0 ? '' : 'd-none'}>
