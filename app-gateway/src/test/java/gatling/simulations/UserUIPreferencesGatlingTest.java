...
/**
 * Performance test for the UserUIPreferences entity.
 */
public class UserUIPreferencesGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all userUIPreferences")
                ...
                .exec(
                    http("Create new userUIPreferences")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_userUIPreferences_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created userUIPreferences").get("${new_userUIPreferences_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created userUIPreferences").delete("${new_userUIPreferences_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the UserUIPreferences entity").exec(scn);
