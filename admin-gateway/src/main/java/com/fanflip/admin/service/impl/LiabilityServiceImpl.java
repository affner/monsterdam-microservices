...
public class LiabilityServiceImpl implements LiabilityService {
    ...
    public Flux<LiabilityDTO> findAll() {
        log.debug("Request to get all Liabilities");
        return liabilityRepository.findAll().map(liabilityMapper::toDto);
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public Flux<LiabilityDTO> search(String query) {
        log.debug("Request to search Liabilities for query {}", query);
        try {
            return liabilitySearchRepository.search(query).map(liabilityMapper::toDto);
