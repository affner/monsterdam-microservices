...
/**
 * REST controller for managing {@link com.monsterdam.admin.domain.TaxDeclaration}.
 */
@RestController
...
public class TaxDeclarationResource {
    ...
    public Mono<List<TaxDeclarationDTO>> getAllTaxDeclarations(
        @RequestParam(name = "eagerload", required = false, defaultValue = "true") boolean eagerload
    ) {
        log.debug("REST request to get all TaxDeclarations");
        return taxDeclarationService.findAll().collectList();
    }

    /**
     * {@code GET  /tax-declarations} : get all the taxDeclarations as a stream.
     * @return the {@link Flux} of taxDeclarations.
     */
    @GetMapping(value = "", produces = MediaType.APPLICATION_NDJSON_VALUE)
    public Flux<TaxDeclarationDTO> getAllTaxDeclarationsAsStream() {
        log.debug("REST request to get all TaxDeclarations as a stream");
        return taxDeclarationService.findAll();
    }
     ...
     */
    @GetMapping("/_search")
    public Mono<List<TaxDeclarationDTO>> searchTaxDeclarations(@RequestParam("query") String query) {
        log.debug("REST request to search TaxDeclarations for query {}", query);
        try {
            return taxDeclarationService.search(query).collectList();
