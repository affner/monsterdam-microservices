            ...
            <tbody>
              {taxDeclarationList.map((taxDeclaration, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {taxDeclaration.accountingRecords
                      ? taxDeclaration.accountingRecords.map((val, j) => (
                          <span key={j}>
                            <Link to={`/accounting-record/${val.id}`}>{val.id}</Link>
                            {j === taxDeclaration.accountingRecords.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/tax-declaration/${taxDeclaration.id}/edit`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() => (window.location.href = `/tax-declaration/${taxDeclaration.id}/delete`)}
                        color="danger"
                        size="sm"
