...
import com.fanflip.admin.repository.search.AdminAnnouncementSearchRepository;
import com.fanflip.admin.service.dto.AdminAnnouncementDTO;
import com.fanflip.admin.service.mapper.AdminAnnouncementMapper;
...
class AdminAnnouncementResourceIT {
    ...
    void createAdminAnnouncement() throws Exception {
        ...
        assertThat(testAdminAnnouncement.getDirectMessageId()).isEqualTo(DEFAULT_DIRECT_MESSAGE_ID);
    }
    ...
    @Test
    void checkDirectMessageIdIsRequired() throws Exception {
        int databaseSizeBeforeTest = adminAnnouncementRepository.findAll().collectList().block().size();
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(adminAnnouncementSearchRepository.findAll().collectList().block());
        // set the field null
        adminAnnouncement.setDirectMessageId(null);
    ...
    @Test
    void getAllAdminAnnouncementsAsStream() {
        // Initialize the database
        adminAnnouncementRepository.save(adminAnnouncement).block();

        List<AdminAnnouncement> adminAnnouncementList = webTestClient
            .get()
            .uri(ENTITY_API_URL)
            ...
            .returnResult(AdminAnnouncementDTO.class)
            .getResponseBody()
            .map(adminAnnouncementMapper::toEntity)
            .filter(adminAnnouncement::equals)
            .collectList()
            .block(Duration.ofSeconds(5));

        assertThat(adminAnnouncementList).isNotNull();
        assertThat(adminAnnouncementList).hasSize(1);
        AdminAnnouncement testAdminAnnouncement = adminAnnouncementList.get(0);
        assertThat(testAdminAnnouncement.getAnnouncementType()).isEqualTo(DEFAULT_ANNOUNCEMENT_TYPE);
        assertThat(testAdminAnnouncement.getTitle()).isEqualTo(DEFAULT_TITLE);
        assertThat(testAdminAnnouncement.getContent()).isEqualTo(DEFAULT_CONTENT);
        assertThat(testAdminAnnouncement.getCreatedDate()).isEqualTo(DEFAULT_CREATED_DATE);
    ...
    void putExistingAdminAnnouncement() throws Exception {
        ...
        assertThat(testAdminAnnouncement.getDirectMessageId()).isEqualTo(UPDATED_DIRECT_MESSAGE_ID);
        await()
            .atMost(5, TimeUnit.SECONDS)
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(adminAnnouncementSearchRepository.findAll().collectList().block());
                ...
                assertThat(testAdminAnnouncementSearch.getDirectMessageId()).isEqualTo(UPDATED_DIRECT_MESSAGE_ID);
            });
    }
    ...
    void partialUpdateAdminAnnouncementWithPatch() throws Exception {
        ...
        partialUpdatedAdminAnnouncement.createdDate(UPDATED_CREATED_DATE).directMessageId(UPDATED_DIRECT_MESSAGE_ID);
