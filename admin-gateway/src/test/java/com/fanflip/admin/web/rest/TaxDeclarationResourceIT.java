...
class TaxDeclarationResourceIT {
    ...
    @Test
    void getAllTaxDeclarationsAsStream() {
        // Initialize the database
        taxDeclarationRepository.save(taxDeclaration).block();

        List<TaxDeclaration> taxDeclarationList = webTestClient
            .get()
            .uri(ENTITY_API_URL)
            ...
            .returnResult(TaxDeclarationDTO.class)
            .getResponseBody()
            .map(taxDeclarationMapper::toEntity)
            .filter(taxDeclaration::equals)
            .collectList()
            .block(Duration.ofSeconds(5));

        assertThat(taxDeclarationList).isNotNull();
        assertThat(taxDeclarationList).hasSize(1);
        TaxDeclaration testTaxDeclaration = taxDeclarationList.get(0);
        assertThat(testTaxDeclaration.getYear()).isEqualTo(DEFAULT_YEAR);
        assertThat(testTaxDeclaration.getDeclarationType()).isEqualTo(DEFAULT_DECLARATION_TYPE);
        assertThat(testTaxDeclaration.getSubmittedDate()).isEqualTo(DEFAULT_SUBMITTED_DATE);
        assertThat(testTaxDeclaration.getStatus()).isEqualTo(DEFAULT_STATUS);
