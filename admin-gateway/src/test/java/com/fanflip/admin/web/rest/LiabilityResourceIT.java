...
class LiabilityResourceIT {
    ...
    @Test
    void getAllLiabilitiesAsStream() {
        // Initialize the database
        liabilityRepository.save(liability).block();

        List<Liability> liabilityList = webTestClient
            .get()
            .uri(ENTITY_API_URL)
            ...
            .returnResult(LiabilityDTO.class)
            .getResponseBody()
            .map(liabilityMapper::toEntity)
            .filter(liability::equals)
            .collectList()
            .block(Duration.ofSeconds(5));

        assertThat(liabilityList).isNotNull();
        assertThat(liabilityList).hasSize(1);
        Liability testLiability = liabilityList.get(0);
        assertThat(testLiability.getName()).isEqualTo(DEFAULT_NAME);
        assertThat(testLiability.getAmount()).isEqualByComparingTo(DEFAULT_AMOUNT);
        assertThat(testLiability.getDueDate()).isEqualTo(DEFAULT_DUE_DATE);
        assertThat(testLiability.getType()).isEqualTo(DEFAULT_TYPE);
