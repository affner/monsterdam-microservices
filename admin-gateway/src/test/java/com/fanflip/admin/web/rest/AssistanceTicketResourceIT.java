...
class AssistanceTicketResourceIT {
    ...
    void createAssistanceTicket() throws Exception {
        ...
        assertThat(testAssistanceTicket.getUserId()).isEqualTo(DEFAULT_USER_ID);
    }
    ...
    @Test
    void getAllAssistanceTicketsAsStream() {
        // Initialize the database
        assistanceTicketRepository.save(assistanceTicket).block();

        List<AssistanceTicket> assistanceTicketList = webTestClient
            .get()
            .uri(ENTITY_API_URL)
            ...
            .returnResult(AssistanceTicketDTO.class)
            .getResponseBody()
            .map(assistanceTicketMapper::toEntity)
            .filter(assistanceTicket::equals)
            .collectList()
            .block(Duration.ofSeconds(5));

        assertThat(assistanceTicketList).isNotNull();
        assertThat(assistanceTicketList).hasSize(1);
        AssistanceTicket testAssistanceTicket = assistanceTicketList.get(0);
        assertThat(testAssistanceTicket.getSubject()).isEqualTo(DEFAULT_SUBJECT);
        assertThat(testAssistanceTicket.getDescription()).isEqualTo(DEFAULT_DESCRIPTION);
        assertThat(testAssistanceTicket.getStatus()).isEqualTo(DEFAULT_STATUS);
        assertThat(testAssistanceTicket.getType()).isEqualTo(DEFAULT_TYPE);
    ...
    void putExistingAssistanceTicket() throws Exception {
        ...
        assertThat(testAssistanceTicket.getUserId()).isEqualTo(UPDATED_USER_ID);
        await()
            .atMost(5, TimeUnit.SECONDS)
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(assistanceTicketSearchRepository.findAll().collectList().block());
                ...
                assertThat(testAssistanceTicketSearch.getUserId()).isEqualTo(UPDATED_USER_ID);
            });
    }
