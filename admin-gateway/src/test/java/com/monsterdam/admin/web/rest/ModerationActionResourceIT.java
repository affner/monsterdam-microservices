...
class ModerationActionResourceIT {
    ...
    @Test
    void getAllModerationActionsAsStream() {
        // Initialize the database
        moderationActionRepository.save(moderationAction).block();

        List<ModerationAction> moderationActionList = webTestClient
            .get()
            .uri(ENTITY_API_URL)
            ...
            .returnResult(ModerationActionDTO.class)
            .getResponseBody()
            .map(moderationActionMapper::toEntity)
            .filter(moderationAction::equals)
            .collectList()
            .block(Duration.ofSeconds(5));

        assertThat(moderationActionList).isNotNull();
        assertThat(moderationActionList).hasSize(1);
        ModerationAction testModerationAction = moderationActionList.get(0);
        assertThat(testModerationAction.getActionType()).isEqualTo(DEFAULT_ACTION_TYPE);
        assertThat(testModerationAction.getReason()).isEqualTo(DEFAULT_REASON);
        assertThat(testModerationAction.getActionDate()).isEqualTo(DEFAULT_ACTION_DATE);
        assertThat(testModerationAction.getDurationDays()).isEqualTo(DEFAULT_DURATION_DAYS);
