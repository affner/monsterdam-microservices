...
/**
 * Performance test for the FinancialStatement entity.
 */
public class FinancialStatementGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all financialStatements")
                ...
                .exec(
                    http("Create new financialStatement")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_financialStatement_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created financialStatement").get("${new_financialStatement_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created financialStatement").delete("${new_financialStatement_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the FinancialStatement entity").exec(scn);
