...
/**
 * Performance test for the AdminUserProfile entity.
 */
public class AdminUserProfileGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all adminUserProfiles")
                ...
                .exec(
                    http("Create new adminUserProfile")
                        .post("/api/admin-user-profiles")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_adminUserProfile_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created adminUserProfile").get("${new_adminUserProfile_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created adminUserProfile").delete("${new_adminUserProfile_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AdminUserProfile entity").exec(scn);
