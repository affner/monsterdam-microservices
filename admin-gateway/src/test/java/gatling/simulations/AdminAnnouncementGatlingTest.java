...
/**
 * Performance test for the AdminAnnouncement entity.
 */
public class AdminAnnouncementGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all adminAnnouncements")
                ...
                .exec(
                    http("Create new adminAnnouncement")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_adminAnnouncement_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created adminAnnouncement").get("${new_adminAnnouncement_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created adminAnnouncement").delete("${new_adminAnnouncement_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AdminAnnouncement entity").exec(scn);
