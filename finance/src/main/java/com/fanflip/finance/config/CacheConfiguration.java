...
public class CacheConfiguration {
    ...
    // Redis-backed cache configuration kept for future phases (currently disabled by default).
    //
    //    @Bean
    ...
    //
    //            if (redisUri.getUserInfo() != null) {
    //                singleServerConfig.setPassword(redisUri.getUserInfo().substring(singleServerConfig.getAddress().indexOf(':') + 1));
    //            }
    //        }
    ...
    @Bean
    @ConditionalOnProperty(prefix = "app.features", name = "redis-enabled", havingValue = "true")
    public javax.cache.configuration.Configuration<Object, Object> jcacheConfiguration(JHipsterProperties jHipsterProperties) {
        throw new IllegalStateException("Redis cache is disabled for MVP; enable app.features.redis-enabled to restore it.");
    }
    ...
    @Bean
    @ConditionalOnProperty(prefix = "app.features", name = "redis-enabled", havingValue = "false", matchIfMissing = true)
