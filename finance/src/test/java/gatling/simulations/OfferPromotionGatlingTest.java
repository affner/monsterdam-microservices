...
public class OfferPromotionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all offerPromotions")
                    .get("/services/finance/api/offer-promotions")
                ...
                .exec(
                    http("Create new offerPromotion")
                        .post("/services/finance/api/offer-promotions")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created offerPromotion")
                            .get("/services/finance${new_offerPromotion_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created offerPromotion")
                        .delete("/services/finance${new_offerPromotion_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
