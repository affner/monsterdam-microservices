        ...
        .on(
            exec(
                http("Get all subscriptionBundles")
                    .get("/services/finance/api/subscription-bundles")
                ...
                .exec(
                    http("Create new subscriptionBundle")
                        .post("/services/finance/api/subscription-bundles")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created subscriptionBundle")
                            .get("/services/finance${new_subscriptionBundle_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created subscriptionBundle")
                        .delete("/services/finance${new_subscriptionBundle_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
