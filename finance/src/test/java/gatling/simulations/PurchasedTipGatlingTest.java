...
public class PurchasedTipGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all purchasedTips")
                    .get("/services/finance/api/purchased-tips")
                ...
                .exec(
                    http("Create new purchasedTip")
                        .post("/services/finance/api/purchased-tips")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created purchasedTip").get("/services/finance${new_purchasedTip_url}").headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created purchasedTip")
                        .delete("/services/finance${new_purchasedTip_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
