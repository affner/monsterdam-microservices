...
public class PurchasedContentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all purchasedContents")
                    .get("/services/finance/api/purchased-contents")
                ...
                .exec(
                    http("Create new purchasedContent")
                        .post("/services/finance/api/purchased-contents")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created purchasedContent")
                            .get("/services/finance${new_purchasedContent_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created purchasedContent")
                        .delete("/services/finance${new_purchasedContent_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
