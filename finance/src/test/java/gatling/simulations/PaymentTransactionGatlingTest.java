 ...
 */
public class PaymentTransactionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all paymentTransactions")
                    .get("/services/finance/api/payment-transactions")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new paymentTransaction")
                        .post("/services/finance/api/payment-transactions")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                ...
                                ", \"cloudTransactionId\": \"SAMPLE_TEXT\"" +
                                ", \"viewerId\": 0" +
                                ", \"paymentMethodId\": 0" +
                                ", \"paymentProviderId\": 0" +
                                "}"
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_paymentTransaction_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created paymentTransaction")
                            .get("/services/finance${new_paymentTransaction_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created paymentTransaction")
                        .delete("/services/finance${new_paymentTransaction_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PaymentTransaction entity").exec(scn);
