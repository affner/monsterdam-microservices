...
/**
 * Performance test for the PurchasedSubscription entity.
 */
public class PurchasedSubscriptionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all purchasedSubscriptions")
                    .get("/services/finance/api/purchased-subscriptions")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new purchasedSubscription")
                        .post("/services/finance/api/purchased-subscriptions")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_purchasedSubscription_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created purchasedSubscription")
                            .get("/services/finance${new_purchasedSubscription_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created purchasedSubscription")
                        .delete("/services/finance${new_purchasedSubscription_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PurchasedSubscription entity").exec(scn);
