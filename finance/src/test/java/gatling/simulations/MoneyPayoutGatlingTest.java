...
public class MoneyPayoutGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all moneyPayouts")
                    .get("/services/finance/api/money-payouts")
                ...
                .exec(
                    http("Create new moneyPayout")
                        .post("/services/finance/api/money-payouts")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .repeat(5)
                .on(
                    exec(http("Get created moneyPayout").get("/services/finance${new_moneyPayout_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(
                    http("Delete created moneyPayout").delete("/services/finance${new_moneyPayout_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );
