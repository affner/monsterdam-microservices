...
public class CreatorEarningGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all creatorEarnings")
                    .get("/services/finance/api/creator-earnings")
                ...
                .exec(
                    http("Create new creatorEarning")
                        .post("/services/finance/api/creator-earnings")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created creatorEarning")
                            .get("/services/finance${new_creatorEarning_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created creatorEarning")
                        .delete("/services/finance${new_creatorEarning_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
