...
describe('AccountingRecord e2e test', () => {
  ...
  const accountingRecordSample = {
    date: '2024-02-29T16:11:15.478Z',
    description: 'though fries',
    balance: 18245.26,
    accountType: 'EQUITY',
  };
        ...
        cy.authenticatedRequest({
          ...
          cy.intercept(
            ...
            },
            {
              statusCode: 200,
              headers: {
                link: '<http://localhost/api/accounting-records?page=0&size=20>; rel="last",<http://localhost/api/accounting-records?page=0&size=20>; rel="first"',
              },
  ...
  describe('new AccountingRecord page', () => {
    ...
    it('should create an instance of AccountingRecord', () => {
      cy.get(`[data-cy="date"]`).type('2024-02-29T11:22');
      cy.get(`[data-cy="date"]`).blur();
      cy.get(`[data-cy="date"]`).should('have.value', '2024-02-29T11:22');

      cy.get(`[data-cy="description"]`).type('chop instead');
      cy.get(`[data-cy="description"]`).should('have.value', 'chop instead');

      cy.get(`[data-cy="debit"]`).type('3527.8');
      cy.get(`[data-cy="debit"]`).should('have.value', '3527.8');

      cy.get(`[data-cy="credit"]`).type('20664.55');
      cy.get(`[data-cy="credit"]`).should('have.value', '20664.55');

      cy.get(`[data-cy="balance"]`).type('9350.23');
      cy.get(`[data-cy="balance"]`).should('have.value', '9350.23');
