...
describe('AdminAnnouncement e2e test', () => {
  ...
  const adminAnnouncementSample = {
    announcementType: 'COMMUNITY_ALERT',
    title: 'how',
    content: 'brr now letter',
    createdDate: '2024-02-29T07:36:59.560Z',
  };
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      body: {
        ...
        lastLoginDate: '2024-02-29T05:44:58.403Z',
        birthDate: '2024-02-29',
        createdDate: '2024-02-29T07:37:20.960Z',
        lastModifiedDate: '2024-02-29T20:14:46.446Z',
        createdBy: 'where',
        ...
        cy.authenticatedRequest({
          ...
          cy.intercept(
            ...
            },
            {
              statusCode: 200,
              headers: {
                link: '<http://localhost/api/admin-announcements?page=0&size=20>; rel="last",<http://localhost/api/admin-announcements?page=0&size=20>; rel="first"',
              },
  ...
  describe('new AdminAnnouncement page', () => {
    ...
    it('should create an instance of AdminAnnouncement', () => {
      cy.get(`[data-cy="announcementType"]`).select('BANNER');

      cy.get(`[data-cy="title"]`).type('than uneven haunting');
      cy.get(`[data-cy="title"]`).should('have.value', 'than uneven haunting');

      cy.get(`[data-cy="content"]`).type('soon shakily');
      cy.get(`[data-cy="content"]`).should('have.value', 'soon shakily');

      cy.get(`[data-cy="createdDate"]`).type('2024-02-29T21:33');
      cy.get(`[data-cy="createdDate"]`).blur();
      cy.get(`[data-cy="createdDate"]`).should('have.value', '2024-02-29T21:33');

      cy.get(`[data-cy="lastModifiedDate"]`).type('2024-02-29T17:08');
      cy.get(`[data-cy="lastModifiedDate"]`).blur();
      cy.get(`[data-cy="lastModifiedDate"]`).should('have.value', '2024-02-29T17:08');

      cy.get(`[data-cy="createdBy"]`).type('gah');
      cy.get(`[data-cy="createdBy"]`).should('have.value', 'gah');

      cy.get(`[data-cy="lastModifiedBy"]`).type('harden rude enumerate');
      cy.get(`[data-cy="lastModifiedBy"]`).should('have.value', 'harden rude enumerate');
