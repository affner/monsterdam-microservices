...
/**
 * Performance test for the SingleDocument entity.
 */
public class SingleDocumentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all singleDocuments").get("/api/single-documents").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new singleDocument")
                        .post("/api/single-documents")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_singleDocument_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created singleDocument").get("${new_singleDocument_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created singleDocument").delete("${new_singleDocument_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SingleDocument entity").exec(scn);
