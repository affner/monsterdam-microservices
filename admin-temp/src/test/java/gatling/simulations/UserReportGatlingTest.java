 ...
 */
public class UserReportGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all userReports").get("/api/user-reports").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new userReport")
                        .post("/api/user-reports")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"reportDescription\": \"SAMPLE_TEXT\"" +
                                ", \"status\": \"PENDING\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_userReport_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created userReport").get("${new_userReport_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created userReport").delete("${new_userReport_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the UserReport entity").exec(scn);
