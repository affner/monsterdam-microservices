...
/**
 * Performance test for the SubscriptionBundle entity.
 */
public class SubscriptionBundleGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all subscriptionBundles")
                    .get("/api/subscription-bundles")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new subscriptionBundle")
                        .post("/api/subscription-bundles")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_subscriptionBundle_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created subscriptionBundle").get("${new_subscriptionBundle_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created subscriptionBundle").delete("${new_subscriptionBundle_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SubscriptionBundle entity").exec(scn);
