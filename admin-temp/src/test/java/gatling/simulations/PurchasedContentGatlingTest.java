...
/**
 * Performance test for the PurchasedContent entity.
 */
public class PurchasedContentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all purchasedContents").get("/api/purchased-contents").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new purchasedContent")
                        .post("/api/purchased-contents")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_purchasedContent_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created purchasedContent").get("${new_purchasedContent_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created purchasedContent").delete("${new_purchasedContent_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PurchasedContent entity").exec(scn);
