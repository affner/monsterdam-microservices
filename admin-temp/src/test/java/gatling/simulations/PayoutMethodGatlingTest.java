...
/**
 * Performance test for the PayoutMethod entity.
 */
public class PayoutMethodGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all payoutMethods").get("/api/payout-methods").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new payoutMethod")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_payoutMethod_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created payoutMethod").get("${new_payoutMethod_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created payoutMethod").delete("${new_payoutMethod_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PayoutMethod entity").exec(scn);
