...
/**
 * Performance test for the VideoStory entity.
 */
public class VideoStoryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all videoStories").get("/api/video-stories").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new videoStory")
                        .post("/api/video-stories")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_videoStory_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created videoStory").get("${new_videoStory_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created videoStory").delete("${new_videoStory_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the VideoStory entity").exec(scn);
