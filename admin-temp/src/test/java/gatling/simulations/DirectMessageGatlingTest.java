...
/**
 * Performance test for the DirectMessage entity.
 */
public class DirectMessageGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all directMessages").get("/api/direct-messages").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new directMessage")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"messageContent\": null" +
                                ", \"readDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"likeCount\": 0" +
                                ", \"isHidden\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_directMessage_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created directMessage").get("${new_directMessage_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created directMessage").delete("${new_directMessage_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DirectMessage entity").exec(scn);
