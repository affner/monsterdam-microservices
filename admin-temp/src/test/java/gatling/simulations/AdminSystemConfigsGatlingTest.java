    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                ...
                .repeat(5)
                .on(
                    exec(http("Get created adminSystemConfigs").get("${new_adminSystemConfigs_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created adminSystemConfigs").delete("${new_adminSystemConfigs_url}").headers(headers_http_authenticated))
                .pause(10)
        );
