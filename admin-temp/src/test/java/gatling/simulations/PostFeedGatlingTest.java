...
public class PostFeedGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all postFeeds").get("/api/post-feeds").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created postFeed").get("${new_postFeed_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created postFeed").delete("${new_postFeed_url}").headers(headers_http_authenticated))
                .pause(10)
        );
