...
/**
 * Performance test for the SpecialReward entity.
 */
public class SpecialRewardGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all specialRewards").get("/api/special-rewards").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new specialReward")
                        .post("/api/special-rewards")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_specialReward_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created specialReward").get("${new_specialReward_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created specialReward").delete("${new_specialReward_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SpecialReward entity").exec(scn);
