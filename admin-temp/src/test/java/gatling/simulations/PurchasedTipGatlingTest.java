...
/**
 * Performance test for the PurchasedTip entity.
 */
public class PurchasedTipGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all purchasedTips").get("/api/purchased-tips").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new purchasedTip")
                        .post("/api/purchased-tips")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_purchasedTip_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created purchasedTip").get("${new_purchasedTip_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created purchasedTip").delete("${new_purchasedTip_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PurchasedTip entity").exec(scn);
