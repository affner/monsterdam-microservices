...
/**
 * Performance test for the SpecialAward entity.
 */
public class SpecialAwardGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all specialAwards").get("/api/special-awards").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new specialAward")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_specialAward_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created specialAward").get("${new_specialAward_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created specialAward").delete("${new_specialAward_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SpecialAward entity").exec(scn);
