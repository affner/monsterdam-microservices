...
/**
 * Performance test for the UserAssociation entity.
 */
public class UserAssociationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all userAssociations").get("/api/user-associations").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new userAssociation")
                        .post("/api/user-associations")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"requestedDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"status\": \"REQUESTED\"" +
                                ", \"associationToken\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_userAssociation_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created userAssociation").get("${new_userAssociation_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created userAssociation").delete("${new_userAssociation_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the UserAssociation entity").exec(scn);
