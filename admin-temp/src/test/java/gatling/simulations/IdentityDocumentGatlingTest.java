...
/**
 * Performance test for the IdentityDocument entity.
 */
public class IdentityDocumentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all identityDocuments").get("/api/identity-documents").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new identityDocument")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_identityDocument_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created identityDocument").get("${new_identityDocument_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created identityDocument").delete("${new_identityDocument_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the IdentityDocument entity").exec(scn);
