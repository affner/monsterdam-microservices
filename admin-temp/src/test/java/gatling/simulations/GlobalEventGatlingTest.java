...
/**
 * Performance test for the GlobalEvent entity.
 */
public class GlobalEventGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all globalEvents").get("/api/global-events").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new globalEvent")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_globalEvent_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created globalEvent").get("${new_globalEvent_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created globalEvent").delete("${new_globalEvent_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the GlobalEvent entity").exec(scn);
