...
/**
 * Performance test for the CreatorEarning entity.
 */
public class CreatorEarningGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all creatorEarnings").get("/api/creator-earnings").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new creatorEarning")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_creatorEarning_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created creatorEarning").get("${new_creatorEarning_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created creatorEarning").delete("${new_creatorEarning_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CreatorEarning entity").exec(scn);
