...
public class CurrencyGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all currencies").get("/api/currencies").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created currency").get("${new_currency_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created currency").delete("${new_currency_url}").headers(headers_http_authenticated))
                .pause(10)
        );
