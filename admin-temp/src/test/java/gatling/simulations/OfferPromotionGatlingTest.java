 ...
 */
public class OfferPromotionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all offerPromotions").get("/api/offer-promotions").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new offerPromotion")
                        .post("/api/offer-promotions")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"freeDaysDuration\": null" +
                                ", \"discountPercentage\": 0" +
                                ", \"startDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"endDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"subscriptionsLimit\": 0" +
                                ", \"linkCode\": \"SAMPLE_TEXT\"" +
                                ", \"isFinished\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_offerPromotion_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created offerPromotion").get("${new_offerPromotion_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created offerPromotion").delete("${new_offerPromotion_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the OfferPromotion entity").exec(scn);
