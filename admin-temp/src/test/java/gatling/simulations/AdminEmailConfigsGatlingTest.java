    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                ...
                .repeat(5)
                .on(
                    exec(http("Get created adminEmailConfigs").get("${new_adminEmailConfigs_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created adminEmailConfigs").delete("${new_adminEmailConfigs_url}").headers(headers_http_authenticated))
                .pause(10)
        );
