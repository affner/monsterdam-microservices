...
/**
 * Performance test for the EmojiType entity.
 */
public class EmojiTypeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all emojiTypes").get("/api/emoji-types").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new emojiType")
                        .post("/api/emoji-types")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_emojiType_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created emojiType").get("${new_emojiType_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created emojiType").delete("${new_emojiType_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the EmojiType entity").exec(scn);
