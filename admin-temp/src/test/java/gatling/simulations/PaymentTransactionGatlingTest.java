    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                ...
                .repeat(5)
                .on(
                    exec(http("Get created paymentTransaction").get("${new_paymentTransaction_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created paymentTransaction").delete("${new_paymentTransaction_url}").headers(headers_http_authenticated))
                .pause(10)
        );
