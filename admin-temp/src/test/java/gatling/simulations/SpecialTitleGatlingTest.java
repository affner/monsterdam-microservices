...
/**
 * Performance test for the SpecialTitle entity.
 */
public class SpecialTitleGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all specialTitles").get("/api/special-titles").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new specialTitle")
                        .post("/api/special-titles")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_specialTitle_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created specialTitle").get("${new_specialTitle_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created specialTitle").delete("${new_specialTitle_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SpecialTitle entity").exec(scn);
