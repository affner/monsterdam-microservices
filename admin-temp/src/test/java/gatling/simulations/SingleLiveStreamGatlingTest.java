...
/**
 * Performance test for the SingleLiveStream entity.
 */
public class SingleLiveStreamGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all singleLiveStreams")
                    .get("/api/single-live-streams")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new singleLiveStream")
                        .post("/api/single-live-streams")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_singleLiveStream_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created singleLiveStream").get("${new_singleLiveStream_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created singleLiveStream").delete("${new_singleLiveStream_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SingleLiveStream entity").exec(scn);
