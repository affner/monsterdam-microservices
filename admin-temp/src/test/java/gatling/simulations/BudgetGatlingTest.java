 ...
 */
public class BudgetGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all budgets").get("/api/budgets").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new budget")
                        .post("/api/budgets")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"spentAmount\": 0" +
                                ", \"remainingAmount\": 0" +
                                ", \"budgetDetails\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_budget_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created budget").get("${new_budget_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created budget").delete("${new_budget_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Budget entity").exec(scn);
