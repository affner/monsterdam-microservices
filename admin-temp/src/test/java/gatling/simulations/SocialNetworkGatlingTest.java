 ...
 */
public class SocialNetworkGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all socialNetworks").get("/api/social-networks").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new socialNetwork")
                        ...
                        .body(
                            StringBody(
                                ...
                                "\"thumbnail\": null" +
                                ", \"name\": \"SAMPLE_TEXT\"" +
                                ", \"completeName\": \"SAMPLE_TEXT\"" +
                                ", \"mainLink\": \"SAMPLE_TEXT\"" +
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created socialNetwork").get("${new_socialNetwork_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created socialNetwork").delete("${new_socialNetwork_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SocialNetwork entity").exec(scn);
