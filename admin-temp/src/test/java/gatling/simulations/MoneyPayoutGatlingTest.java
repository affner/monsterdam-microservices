...
/**
 * Performance test for the MoneyPayout entity.
 */
public class MoneyPayoutGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all moneyPayouts").get("/api/money-payouts").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new moneyPayout")
                        .post("/api/money-payouts")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_moneyPayout_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created moneyPayout").get("${new_moneyPayout_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created moneyPayout").delete("${new_moneyPayout_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MoneyPayout entity").exec(scn);
