 ...
 */
public class PostCommentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all postComments").get("/api/post-comments").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new postComment")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created postComment").get("${new_postComment_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created postComment").delete("${new_postComment_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PostComment entity").exec(scn);
