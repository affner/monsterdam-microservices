...
/**
 * Performance test for the AssistanceTicket entity.
 */
public class AssistanceTicketGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all assistanceTickets").get("/api/assistance-tickets").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new assistanceTicket")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_assistanceTicket_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created assistanceTicket").get("${new_assistanceTicket_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created assistanceTicket").delete("${new_assistanceTicket_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AssistanceTicket entity").exec(scn);
