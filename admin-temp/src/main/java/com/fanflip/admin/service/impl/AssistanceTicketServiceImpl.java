...
public class AssistanceTicketServiceImpl implements AssistanceTicketService {
    ...
    public Flux<AssistanceTicketDTO> findAll(Pageable pageable) {
        log.debug("Request to get all AssistanceTickets");
        return assistanceTicketRepository.findAllBy(pageable).map(assistanceTicketMapper::toDto);
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public Flux<AssistanceTicketDTO> search(String query, Pageable pageable) {
        log.debug("Request to search for a page of AssistanceTickets for query {}", query);
        return assistanceTicketSearchRepository.search(query, pageable).map(assistanceTicketMapper::toDto);
    }
}
