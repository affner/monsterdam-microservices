...
public class IdentityDocumentServiceImpl implements IdentityDocumentService {
    ...
    public Flux<IdentityDocumentDTO> findAll(Pageable pageable) {
        log.debug("Request to get all IdentityDocuments");
        return identityDocumentRepository.findAllBy(pageable).map(identityDocumentMapper::toDto);
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public Flux<IdentityDocumentDTO> search(String query, Pageable pageable) {
        log.debug("Request to search for a page of IdentityDocuments for query {}", query);
        return identityDocumentSearchRepository.search(query, pageable).map(identityDocumentMapper::toDto);
    }
}
