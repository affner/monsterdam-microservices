...
/**
 * Service Implementation for managing {@link com.fanflip.admin.domain.AccountingRecord}.
 */
@Service
@Transactional
public class AccountingRecordServiceImpl implements AccountingRecordService {
    ...
    public Flux<AccountingRecordDTO> findAll(Pageable pageable) {
        log.debug("Request to get all AccountingRecords");
        return accountingRecordRepository.findAllBy(pageable).map(accountingRecordMapper::toDto);
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public Flux<AccountingRecordDTO> search(String query, Pageable pageable) {
        log.debug("Request to search for a page of AccountingRecords for query {}", query);
        return accountingRecordSearchRepository.search(query, pageable).map(accountingRecordMapper::toDto);
    }
}
