...
/**
 * Service Implementation for managing {@link com.monsterdam.admin.domain.Budget}.
 */
@Service
@Transactional
public class BudgetServiceImpl implements BudgetService {
    ...
    public Flux<BudgetDTO> findAll(Pageable pageable) {
        log.debug("Request to get all Budgets");
        return budgetRepository.findAllBy(pageable).map(budgetMapper::toDto);
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public Flux<BudgetDTO> search(String query, Pageable pageable) {
        log.debug("Request to search for a page of Budgets for query {}", query);
        return budgetSearchRepository.search(query, pageable).map(budgetMapper::toDto);
    }
}
