...
public class AdminUserProfileServiceImpl implements AdminUserProfileService {
    ...
    public Flux<AdminUserProfileDTO> findAll(Pageable pageable) {
        log.debug("Request to get all AdminUserProfiles");
        return adminUserProfileRepository.findAllBy(pageable).map(adminUserProfileMapper::toDto);
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public Flux<AdminUserProfileDTO> search(String query, Pageable pageable) {
        log.debug("Request to search for a page of AdminUserProfiles for query {}", query);
        return adminUserProfileSearchRepository.search(query, pageable).map(adminUserProfileMapper::toDto);
    }
}
