...
import com.fanflip.admin.repository.search.FinancialStatementSearchRepository;
import com.fanflip.admin.service.FinancialStatementService;
import com.fanflip.admin.service.dto.FinancialStatementDTO;
import com.fanflip.admin.service.mapper.FinancialStatementMapper;
...
/**
 * Service Implementation for managing {@link com.fanflip.admin.domain.FinancialStatement}.
 */
@Service
@Transactional
public class FinancialStatementServiceImpl implements FinancialStatementService {
    ...
    public Flux<FinancialStatementDTO> findAll(Pageable pageable) {
        log.debug("Request to get all FinancialStatements");
        return financialStatementRepository.findAllBy(pageable).map(financialStatementMapper::toDto);
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public Flux<FinancialStatementDTO> search(String query, Pageable pageable) {
        log.debug("Request to search for a page of FinancialStatements for query {}", query);
        return financialStatementSearchRepository.search(query, pageable).map(financialStatementMapper::toDto);
    }
}
