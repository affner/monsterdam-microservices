...
public class AssetServiceImpl implements AssetService {
    ...
    public Flux<AssetDTO> findAll(Pageable pageable) {
        log.debug("Request to get all Assets");
        return assetRepository.findAllBy(pageable).map(assetMapper::toDto);
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public Flux<AssetDTO> search(String query, Pageable pageable) {
        log.debug("Request to search for a page of Assets for query {}", query);
        return assetSearchRepository.search(query, pageable).map(assetMapper::toDto);
    }
}
