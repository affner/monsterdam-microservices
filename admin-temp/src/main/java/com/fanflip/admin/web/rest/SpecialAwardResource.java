...
/**
 * REST controller for managing {@link com.monsterdam.admin.domain.SpecialAward}.
 */
@RestController
...
public class SpecialAwardResource {
     ...
     */
    @PostMapping("")
    public Mono<ResponseEntity<SpecialAwardDTO>> createSpecialAward(@Valid @RequestBody SpecialAwardDTO specialAwardDTO)
        ...
        return specialAwardService
            .save(specialAwardDTO)
            .map(result -> {
                try {
     ...
     */
    @PutMapping("/{id}")
    public Mono<ResponseEntity<SpecialAwardDTO>> updateSpecialAward(
    ...
    public Mono<ResponseEntity<SpecialAwardDTO>> partialUpdateSpecialAward(
        @PathVariable(value = "id", required = false) final Long id,
        ...
        return specialAwardRepository
            ...
            .flatMap(exists -> {
                ...
                Mono<SpecialAwardDTO> result = specialAwardService.partialUpdate(specialAwardDTO);
    ...
    public Mono<ResponseEntity<List<SpecialAwardDTO>>> getAllSpecialAwards(
        ...
        return specialAwardService
            .countAll()
            .zipWith(specialAwardService.findAll(pageable).collectList())
     ...
     */
    @GetMapping("/{id}")
    public Mono<ResponseEntity<SpecialAwardDTO>> getSpecialAward(@PathVariable("id") Long id) {
        log.debug("REST request to get SpecialAward : {}", id);
        Mono<SpecialAwardDTO> specialAwardDTO = specialAwardService.findOne(id);
     ...
     */
    @DeleteMapping("/{id}")
    public Mono<ResponseEntity<Void>> deleteSpecialAward(@PathVariable("id") Long id) {
    ...
    /**
     * {@code SEARCH  /special-awards/_search?query=:query} : search for the specialAward corresponding
     * to the query.
     *
     * @param query the query of the specialAward search.
     * @param pageable the pagination information.
     * @param request a {@link ServerHttpRequest} request.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public Mono<ResponseEntity<Flux<SpecialAwardDTO>>> searchSpecialAwards(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable,
        ServerHttpRequest request
    ) {
        log.debug("REST request to search for a page of SpecialAwards for query {}", query);
        return specialAwardService
            .searchCount()
            ...
            .map(headers -> ResponseEntity.ok().headers(headers).body(specialAwardService.search(query, pageable)));
    }
}
