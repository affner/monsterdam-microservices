...
import com.monsterdam.admin.service.UserEventService;
...
/**
 * REST controller for managing {@link com.monsterdam.admin.domain.UserEvent}.
 */
@RestController
...
public class UserEventResource {
     ...
     */
    @PostMapping("")
    public Mono<ResponseEntity<UserEventDTO>> createUserEvent(@Valid @RequestBody UserEventDTO userEventDTO) throws URISyntaxException {
     ...
     */
    @PutMapping("/{id}")
    public Mono<ResponseEntity<UserEventDTO>> updateUserEvent(
    ...
    public Mono<ResponseEntity<UserEventDTO>> partialUpdateUserEvent(
        @PathVariable(value = "id", required = false) final Long id,
        ...
        return userEventRepository
            ...
            .flatMap(exists -> {
                ...
                Mono<UserEventDTO> result = userEventService.partialUpdate(userEventDTO);
    ...
    public Mono<ResponseEntity<List<UserEventDTO>>> getAllUserEvents(
        ...
        return userEventService
            .countAll()
            .zipWith(userEventService.findAll(pageable).collectList())
     ...
     */
    @GetMapping("/{id}")
    public Mono<ResponseEntity<UserEventDTO>> getUserEvent(@PathVariable("id") Long id) {
        log.debug("REST request to get UserEvent : {}", id);
        Mono<UserEventDTO> userEventDTO = userEventService.findOne(id);
     ...
     */
    @DeleteMapping("/{id}")
    public Mono<ResponseEntity<Void>> deleteUserEvent(@PathVariable("id") Long id) {
    ...
    /**
     * {@code SEARCH  /user-events/_search?query=:query} : search for the userEvent corresponding
     * to the query.
     *
     * @param query the query of the userEvent search.
     * @param pageable the pagination information.
     * @param request a {@link ServerHttpRequest} request.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public Mono<ResponseEntity<Flux<UserEventDTO>>> searchUserEvents(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable,
        ServerHttpRequest request
    ) {
        log.debug("REST request to search for a page of UserEvents for query {}", query);
        return userEventService
            .searchCount()
            ...
            .map(headers -> ResponseEntity.ok().headers(headers).body(userEventService.search(query, pageable)));
    }
}
