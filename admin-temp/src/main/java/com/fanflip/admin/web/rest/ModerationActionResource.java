...
/**
 * REST controller for managing {@link com.fanflip.admin.domain.ModerationAction}.
 */
@RestController
...
public class ModerationActionResource {
    ...
    public Mono<ResponseEntity<List<ModerationActionDTO>>> getAllModerationActions(
        ...
        if ("assistanceticket-is-null".equals(filter)) {
            ...
            return moderationActionService.findAllWhereAssistanceTicketIsNull().collectList().map(ResponseEntity::ok);
        }
        log.debug("REST request to get a page of ModerationActions");
        return moderationActionService
            .countAll()
            .zipWith(moderationActionService.findAll(pageable).collectList())
     ...
     */
    @GetMapping("/_search")
    public Mono<ResponseEntity<Flux<ModerationActionDTO>>> searchModerationActions(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable,
        ServerHttpRequest request
    ) {
        log.debug("REST request to search for a page of ModerationActions for query {}", query);
        return moderationActionService
            .searchCount()
            ...
            .map(headers -> ResponseEntity.ok().headers(headers).body(moderationActionService.search(query, pageable)));
    }
}
