...
/**
 * REST controller for managing {@link com.monsterdam.admin.domain.IdentityDocument}.
 */
@RestController
...
public class IdentityDocumentResource {
    ...
    public Mono<ResponseEntity<List<IdentityDocumentDTO>>> getAllIdentityDocuments(
        ...
        return identityDocumentService
            .countAll()
            .zipWith(identityDocumentService.findAll(pageable).collectList())
     ...
     */
    @GetMapping("/_search")
    public Mono<ResponseEntity<Flux<IdentityDocumentDTO>>> searchIdentityDocuments(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable,
        ServerHttpRequest request
    ) {
        log.debug("REST request to search for a page of IdentityDocuments for query {}", query);
        return identityDocumentService
            .searchCount()
            ...
            .map(headers -> ResponseEntity.ok().headers(headers).body(identityDocumentService.search(query, pageable)));
    }
}
