...
import com.monsterdam.admin.service.SpecialTitleService;
...
/**
 * REST controller for managing {@link com.monsterdam.admin.domain.SpecialTitle}.
 */
@RestController
...
public class SpecialTitleResource {
    ...
    private static final String ENTITY_NAME = "specialTitle";
     ...
     */
    @PostMapping("")
    public Mono<ResponseEntity<SpecialTitleDTO>> createSpecialTitle(@Valid @RequestBody SpecialTitleDTO specialTitleDTO)
        ...
        return specialTitleService
            .save(specialTitleDTO)
            .map(result -> {
                try {
     ...
     */
    @PutMapping("/{id}")
    public Mono<ResponseEntity<SpecialTitleDTO>> updateSpecialTitle(
    ...
    public Mono<ResponseEntity<SpecialTitleDTO>> partialUpdateSpecialTitle(
        @PathVariable(value = "id", required = false) final Long id,
        ...
        return specialTitleRepository
            ...
            .flatMap(exists -> {
                ...
                Mono<SpecialTitleDTO> result = specialTitleService.partialUpdate(specialTitleDTO);
    ...
    public Mono<ResponseEntity<List<SpecialTitleDTO>>> getAllSpecialTitles(
        ...
        return specialTitleService
            .countAll()
            .zipWith(specialTitleService.findAll(pageable).collectList())
     ...
     */
    @GetMapping("/{id}")
    public Mono<ResponseEntity<SpecialTitleDTO>> getSpecialTitle(@PathVariable("id") Long id) {
        log.debug("REST request to get SpecialTitle : {}", id);
        Mono<SpecialTitleDTO> specialTitleDTO = specialTitleService.findOne(id);
     ...
     */
    @DeleteMapping("/{id}")
    public Mono<ResponseEntity<Void>> deleteSpecialTitle(@PathVariable("id") Long id) {
    ...
    /**
     * {@code SEARCH  /special-titles/_search?query=:query} : search for the specialTitle corresponding
     * to the query.
     *
     * @param query the query of the specialTitle search.
     * @param pageable the pagination information.
     * @param request a {@link ServerHttpRequest} request.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public Mono<ResponseEntity<Flux<SpecialTitleDTO>>> searchSpecialTitles(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable,
        ServerHttpRequest request
    ) {
        log.debug("REST request to search for a page of SpecialTitles for query {}", query);
        return specialTitleService
            .searchCount()
            ...
            .map(headers -> ResponseEntity.ok().headers(headers).body(specialTitleService.search(query, pageable)));
    }
}
