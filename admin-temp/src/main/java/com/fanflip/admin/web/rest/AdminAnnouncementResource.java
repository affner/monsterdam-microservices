...
/**
 * REST controller for managing {@link com.monsterdam.admin.domain.AdminAnnouncement}.
 */
@RestController
...
public class AdminAnnouncementResource {
    ...
    public Mono<ResponseEntity<List<AdminAnnouncementDTO>>> getAllAdminAnnouncements(
        ...
        return adminAnnouncementService
            .countAll()
            .zipWith(adminAnnouncementService.findAll(pageable).collectList())
     ...
     */
    @GetMapping("/_search")
    public Mono<ResponseEntity<Flux<AdminAnnouncementDTO>>> searchAdminAnnouncements(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable,
        ServerHttpRequest request
    ) {
        log.debug("REST request to search for a page of AdminAnnouncements for query {}", query);
        return adminAnnouncementService
            .searchCount()
            ...
            .map(headers -> ResponseEntity.ok().headers(headers).body(adminAnnouncementService.search(query, pageable)));
    }
}
