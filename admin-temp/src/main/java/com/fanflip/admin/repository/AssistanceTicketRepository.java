...
public interface AssistanceTicketRepository extends ReactiveCrudRepository<AssistanceTicket, Long>, AssistanceTicketRepositoryInternal {
    ...
    @Query("SELECT * FROM assistance_ticket entity WHERE entity.user_id = :id")
    Flux<AssistanceTicket> findByUser(Long id);

    @Query("SELECT * FROM assistance_ticket entity WHERE entity.user_id IS NULL")
    Flux<AssistanceTicket> findAllWhereUserIsNull();
