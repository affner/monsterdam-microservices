...
import { searchEntities, getEntities } from './user-lite.reducer';
...
export const UserLite = () => {
  ...
  const userLiteList = useAppSelector(state => state.admin.userLite.entities);
  const loading = useAppSelector(state => state.admin.userLite.loading);
  const totalItems = useAppSelector(state => state.admin.userLite.totalItems);
  ...
  return (
    <div>
      <h2 id="user-lite-heading" data-cy="UserLiteHeading">
        <Translate contentKey="adminApp.userLite.home.title">User Lites</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="adminApp.userLite.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/user-lite/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="adminApp.userLite.home.createLabel">Create new User Lite</Translate>
          </Link>
        </div>
      </h2>
      <Row>
            ...
            <FormGroup>
              <InputGroup>
                <Input
                  ...
                  placeholder={translate('adminApp.userLite.home.search')}
                />
        ...
        {userLiteList && userLiteList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="adminApp.userLite.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('contentPreference')}>
                  <Translate contentKey="adminApp.userLite.contentPreference">Content Preference</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('contentPreference')} />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {userLiteList.map((userLite, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`adminApp.UserGender.${userLite.gender}`} />
                  </td>
                  ...
                  <td>{userLite.isDeleted ? 'true' : 'false'}</td>
                  <td>{userLite.nickName}</td>
                  <td>{userLite.fullName}</td>
                  <td>
                    <Translate contentKey={`adminApp.ContentPreference.${userLite.contentPreference}`} />
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="adminApp.userLite.home.notFound">No User Lites found</Translate>
            </div>
          )
        )}
      </div>
