...
import { searchEntities, getEntities } from './single-video.reducer';
...
export const SingleVideo = () => {
  ...
  const singleVideoList = useAppSelector(state => state.admin.singleVideo.entities);
  const loading = useAppSelector(state => state.admin.singleVideo.loading);
  const totalItems = useAppSelector(state => state.admin.singleVideo.totalItems);
  ...
  return (
    <div>
      <h2 id="single-video-heading" data-cy="SingleVideoHeading">
        <Translate contentKey="adminApp.singleVideo.home.title">Single Videos</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="adminApp.singleVideo.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/single-video/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="adminApp.singleVideo.home.createLabel">Create new Single Video</Translate>
          </Link>
        </div>
      </h2>
      <Row>
            ...
            <FormGroup>
              <InputGroup>
                <Input
                  ...
                  placeholder={translate('adminApp.singleVideo.home.search')}
                />
        ...
        {singleVideoList && singleVideoList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="adminApp.singleVideo.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="adminApp.singleVideo.belongPackage">Belong Package</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {singleVideoList.map((singleVideo, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{singleVideo.isDeleted ? 'true' : 'false'}</td>
                  <td>
                    {singleVideo.belongPackage ? (
                      <Link to={`/content-package/${singleVideo.belongPackage.id}`}>{singleVideo.belongPackage.amount}</Link>
                    ) : (
                      ''
                    )}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="adminApp.singleVideo.home.notFound">No Single Videos found</Translate>
            </div>
          )
        )}
      </div>
