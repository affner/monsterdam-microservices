...
import { searchEntities, getEntities } from './book-mark.reducer';
...
export const BookMark = () => {
  ...
  const bookMarkList = useAppSelector(state => state.admin.bookMark.entities);
  const loading = useAppSelector(state => state.admin.bookMark.loading);
  const totalItems = useAppSelector(state => state.admin.bookMark.totalItems);
  ...
  return (
    <div>
      <h2 id="book-mark-heading" data-cy="BookMarkHeading">
        <Translate contentKey="adminApp.bookMark.home.title">Book Marks</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="adminApp.bookMark.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/book-mark/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="adminApp.bookMark.home.createLabel">Create new Book Mark</Translate>
          </Link>
        </div>
      </h2>
      <Row>
            ...
            <FormGroup>
              <InputGroup>
                <Input
                  ...
                  placeholder={translate('adminApp.bookMark.home.search')}
                />
        ...
        {bookMarkList && bookMarkList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="adminApp.bookMark.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="adminApp.bookMark.post">Post</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="adminApp.bookMark.message">Message</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="adminApp.bookMark.user">User</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {bookMarkList.map((bookMark, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{bookMark.isDeleted ? 'true' : 'false'}</td>
                  <td>{bookMark.post ? <Link to={`/post-feed/${bookMark.post.id}`}>{bookMark.post.postContent}</Link> : ''}</td>
                  <td>
                    {bookMark.message ? <Link to={`/direct-message/${bookMark.message.id}`}>{bookMark.message.messageContent}</Link> : ''}
                  </td>
                  <td>{bookMark.user ? <Link to={`/user-profile/${bookMark.user.id}`}>{bookMark.user.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="adminApp.bookMark.home.notFound">No Book Marks found</Translate>
            </div>
          )
        )}
      </div>
