...
import { searchEntities, getEntities } from './video-story.reducer';
...
export const VideoStory = () => {
  ...
  const videoStoryList = useAppSelector(state => state.admin.videoStory.entities);
  const loading = useAppSelector(state => state.admin.videoStory.loading);
  const totalItems = useAppSelector(state => state.admin.videoStory.totalItems);
  ...
  return (
    <div>
      <h2 id="video-story-heading" data-cy="VideoStoryHeading">
        <Translate contentKey="adminApp.videoStory.home.title">Video Stories</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="adminApp.videoStory.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/video-story/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="adminApp.videoStory.home.createLabel">Create new Video Story</Translate>
          </Link>
        </div>
      </h2>
      <Row>
            ...
            <FormGroup>
              <InputGroup>
                <Input
                  ...
                  placeholder={translate('adminApp.videoStory.home.search')}
                />
        ...
        {videoStoryList && videoStoryList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="adminApp.videoStory.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="adminApp.videoStory.creator">Creator</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {videoStoryList.map((videoStory, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {videoStory.thumbnail ? (
                      <div>
                        {videoStory.thumbnailContentType ? (
                          <a onClick={openFile(videoStory.thumbnailContentType, videoStory.thumbnail)}>
                            <img
                              src={`data:${videoStory.thumbnailContentType};base64,${videoStory.thumbnail}`}
                        ...
                        <span>
                          {videoStory.thumbnailContentType}, {byteSize(videoStory.thumbnail)}
                        </span>
                      </div>
                    ) : null}
                  </td>
                  ...
                  <td>
                    {videoStory.content ? (
                      <div>
                        {videoStory.contentContentType ? (
                          <a onClick={openFile(videoStory.contentContentType, videoStory.content)}>
                            <Translate contentKey="entity.action.open">Open</Translate>
                            &nbsp;
                          </a>
                        ) : null}
                        <span>
                          {videoStory.contentContentType}, {byteSize(videoStory.content)}
                        </span>
                      </div>
                    ) : null}
                  </td>
                  ...
                  <td>{videoStory.isDeleted ? 'true' : 'false'}</td>
                  <td>{videoStory.creator ? <Link to={`/user-profile/${videoStory.creator.id}`}>{videoStory.creator.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="adminApp.videoStory.home.notFound">No Video Stories found</Translate>
            </div>
          )
        )}
      </div>
