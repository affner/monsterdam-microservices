...
import { searchEntities, getEntities } from './single-photo.reducer';
...
export const SinglePhoto = () => {
  ...
  const singlePhotoList = useAppSelector(state => state.admin.singlePhoto.entities);
  const loading = useAppSelector(state => state.admin.singlePhoto.loading);
  const totalItems = useAppSelector(state => state.admin.singlePhoto.totalItems);
  ...
  return (
    <div>
      <h2 id="single-photo-heading" data-cy="SinglePhotoHeading">
        <Translate contentKey="adminApp.singlePhoto.home.title">Single Photos</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="adminApp.singlePhoto.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/single-photo/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="adminApp.singlePhoto.home.createLabel">Create new Single Photo</Translate>
          </Link>
        </div>
      </h2>
      <Row>
            ...
            <FormGroup>
              <InputGroup>
                <Input
                  ...
                  placeholder={translate('adminApp.singlePhoto.home.search')}
                />
        ...
        {singlePhotoList && singlePhotoList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="adminApp.singlePhoto.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="adminApp.singlePhoto.belongPackage">Belong Package</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {singlePhotoList.map((singlePhoto, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{singlePhoto.isDeleted ? 'true' : 'false'}</td>
                  <td>
                    {singlePhoto.belongPackage ? (
                      <Link to={`/content-package/${singlePhoto.belongPackage.id}`}>{singlePhoto.belongPackage.amount}</Link>
                    ) : (
                      ''
                    )}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="adminApp.singlePhoto.home.notFound">No Single Photos found</Translate>
            </div>
          )
        )}
      </div>
