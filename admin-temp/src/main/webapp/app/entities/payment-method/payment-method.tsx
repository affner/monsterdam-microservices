...
import { searchEntities, getEntities } from './payment-method.reducer';
...
export const PaymentMethod = () => {
  ...
  const paymentMethodList = useAppSelector(state => state.admin.paymentMethod.entities);
  const loading = useAppSelector(state => state.admin.paymentMethod.loading);
  const totalItems = useAppSelector(state => state.admin.paymentMethod.totalItems);
  ...
  return (
    <div>
      <h2 id="payment-method-heading" data-cy="PaymentMethodHeading">
        <Translate contentKey="adminApp.paymentMethod.home.title">Payment Methods</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="adminApp.paymentMethod.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/payment-method/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            ...
            &nbsp;
            <Translate contentKey="adminApp.paymentMethod.home.createLabel">Create new Payment Method</Translate>
          </Link>
        </div>
      </h2>
      <Row>
            ...
            <FormGroup>
              <InputGroup>
                <Input
                  ...
                  placeholder={translate('adminApp.paymentMethod.home.search')}
                />
        ...
        {paymentMethodList && paymentMethodList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="adminApp.paymentMethod.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('tokenText')}>
                  <Translate contentKey="adminApp.paymentMethod.tokenText">Token Text</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('tokenText')} />
                </th>
                ...
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {paymentMethod.createdDate ? (
                      <TextFormat type="date" value={paymentMethod.createdDate} format={APP_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td>
                    {paymentMethod.lastModifiedDate ? (
                      <TextFormat type="date" value={paymentMethod.lastModifiedDate} format={APP_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td>{paymentMethod.createdBy}</td>
                  <td>{paymentMethod.lastModifiedBy}</td>
                  <td>{paymentMethod.isDeleted ? 'true' : 'false'}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="adminApp.paymentMethod.home.notFound">No Payment Methods found</Translate>
            </div>
          )
        )}
      </div>
