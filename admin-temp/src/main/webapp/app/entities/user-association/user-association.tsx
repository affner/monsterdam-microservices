...
import { searchEntities, getEntities } from './user-association.reducer';
...
export const UserAssociation = () => {
  ...
  const userAssociationList = useAppSelector(state => state.admin.userAssociation.entities);
  const loading = useAppSelector(state => state.admin.userAssociation.loading);
  const totalItems = useAppSelector(state => state.admin.userAssociation.totalItems);
  ...
  return (
    <div>
      <h2 id="user-association-heading" data-cy="UserAssociationHeading">
        <Translate contentKey="adminApp.userAssociation.home.title">User Associations</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="adminApp.userAssociation.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/user-association/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            ...
            &nbsp;
            <Translate contentKey="adminApp.userAssociation.home.createLabel">Create new User Association</Translate>
          </Link>
        </div>
      </h2>
      <Row>
            ...
            <FormGroup>
              <InputGroup>
                <Input
                  ...
                  placeholder={translate('adminApp.userAssociation.home.search')}
                />
        ...
        {userAssociationList && userAssociationList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="adminApp.userAssociation.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="adminApp.userAssociation.owner">Owner</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {userAssociationList.map((userAssociation, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`adminApp.AssociationStatus.${userAssociation.status}`} />
                  </td>
                  ...
                  <td>
                    {userAssociation.owner ? <Link to={`/user-profile/${userAssociation.owner.id}`}>{userAssociation.owner.id}</Link> : ''}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="adminApp.userAssociation.home.notFound">No User Associations found</Translate>
            </div>
          )
        )}
      </div>
