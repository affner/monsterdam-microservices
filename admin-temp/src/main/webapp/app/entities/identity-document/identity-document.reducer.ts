...
const apiSearchUrl = 'api/identity-documents/_search';

// Actions

export const searchEntities = createAsyncThunk('identityDocument/search_entity', async ({ query, page, size, sort }: IQueryParams) => {
  const requestUrl = `${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`;
  return axios.get<IIdentityDocument[]>(requestUrl);
});

export const getEntities = createAsyncThunk('identityDocument/fetch_entity_list', async ({ page, size, sort }: IQueryParams) => {
