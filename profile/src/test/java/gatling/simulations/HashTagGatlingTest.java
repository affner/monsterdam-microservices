...
/**
 * Performance test for the HashTag entity.
 */
public class HashTagGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all hashTags").get("/services/profile/api/hash-tags").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new hashTag")
                        .post("/services/profile/api/hash-tags")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_hashTag_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created hashTag").get("/services/profile${new_hashTag_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created hashTag").delete("/services/profile${new_hashTag_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the HashTag entity").exec(scn);
