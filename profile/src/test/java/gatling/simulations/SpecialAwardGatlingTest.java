...
public class SpecialAwardGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all specialAwards")
                    .get("/services/profile/api/special-awards")
                ...
                .exec(
                    http("Create new specialAward")
                        .post("/services/profile/api/special-awards")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created specialAward").get("/services/profile${new_specialAward_url}").headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created specialAward")
                        .delete("/services/profile${new_specialAward_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
