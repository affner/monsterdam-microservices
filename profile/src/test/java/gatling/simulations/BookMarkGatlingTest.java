 ...
 */
public class BookMarkGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all bookMarks")
                    .get("/services/profile/api/book-marks")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new bookMark")
                        .post("/services/profile/api/book-marks")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_bookMark_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created bookMark").get("/services/profile${new_bookMark_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created bookMark").delete("/services/profile${new_bookMark_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the BookMark entity").exec(scn);
