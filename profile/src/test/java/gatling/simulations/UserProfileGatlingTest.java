...
public class UserProfileGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all userProfiles")
                    .get("/services/profile/api/user-profiles")
                ...
                .exec(
                    http("Create new userProfile")
                        .post("/services/profile/api/user-profiles")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"emailContact\": \"SAMPLE_TEXT\"" +
                                ", \"profilePhoto\": null" +
                                ", \"coverPhoto\": null" +
                                ", \"profilePhotoS3Key\": \"SAMPLE_TEXT\"" +
                                ", \"coverPhotoS3Key\": \"SAMPLE_TEXT\"" +
                                ", \"mainContentUrl\": \"SAMPLE_TEXT\"" +
                                ", \"mobilePhone\": \"SAMPLE_TEXT\"" +
                                ", \"websiteUrl\": \"SAMPLE_TEXT\"" +
                                ", \"amazonWishlistUrl\": \"SAMPLE_TEXT\"" +
                ...
                .repeat(5)
                .on(
                    exec(http("Get created userProfile").get("/services/profile${new_userProfile_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(
                    http("Delete created userProfile").delete("/services/profile${new_userProfile_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );
