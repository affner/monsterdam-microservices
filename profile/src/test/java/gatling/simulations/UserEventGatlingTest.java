...
/**
 * Performance test for the UserEvent entity.
 */
public class UserEventGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all userEvents")
                    .get("/services/profile/api/user-events")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new userEvent")
                        .post("/services/profile/api/user-events")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_userEvent_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created userEvent").get("/services/profile${new_userEvent_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created userEvent").delete("/services/profile${new_userEvent_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the UserEvent entity").exec(scn);
