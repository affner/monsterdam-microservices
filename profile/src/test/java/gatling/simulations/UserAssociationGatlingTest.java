...
public class UserAssociationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all userAssociations")
                    .get("/services/profile/api/user-associations")
                ...
                .exec(
                    http("Create new userAssociation")
                        .post("/services/profile/api/user-associations")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created userAssociation")
                            .get("/services/profile${new_userAssociation_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created userAssociation")
                        .delete("/services/profile${new_userAssociation_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
