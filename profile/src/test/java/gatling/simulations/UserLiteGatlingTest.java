...
/**
 * Performance test for the UserLite entity.
 */
public class UserLiteGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all userLites")
                    .get("/services/profile/api/user-lites")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new userLite")
                        .post("/services/profile/api/user-lites")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_userLite_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created userLite").get("/services/profile${new_userLite_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created userLite").delete("/services/profile${new_userLite_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the UserLite entity").exec(scn);
