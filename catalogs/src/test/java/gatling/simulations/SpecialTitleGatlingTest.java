...
public class SpecialTitleGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all specialTitles")
                    .get("/services/catalogs/api/special-titles")
                ...
                .exec(
                    http("Create new specialTitle")
                        .post("/services/catalogs/api/special-titles")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created specialTitle")
                            .get("/services/catalogs${new_specialTitle_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created specialTitle")
                        .delete("/services/catalogs${new_specialTitle_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
