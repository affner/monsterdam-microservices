...
public class SocialNetworkGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all socialNetworks")
                    .get("/services/catalogs/api/social-networks")
                ...
                .exec(
                    http("Create new socialNetwork")
                        .post("/services/catalogs/api/social-networks")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created socialNetwork")
                            .get("/services/catalogs${new_socialNetwork_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created socialNetwork")
                        .delete("/services/catalogs${new_socialNetwork_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
