...
public class HelpCategoryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all helpCategories")
                    .get("/services/catalogs/api/help-categories")
                ...
                .exec(
                    http("Create new helpCategory")
                        .post("/services/catalogs/api/help-categories")
                        .headers(headers_http_authenticated)
                ...
                .on(
                    exec(
                        http("Get created helpCategory")
                            .get("/services/catalogs${new_helpCategory_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created helpCategory")
                        .delete("/services/catalogs${new_helpCategory_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
