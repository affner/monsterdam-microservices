...
/**
 * Performance test for the TaxInfo entity.
 */
public class TaxInfoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all taxInfos").get("/services/catalogs/api/tax-infos").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new taxInfo")
                        .post("/services/catalogs/api/tax-infos")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_taxInfo_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created taxInfo").get("/services/catalogs${new_taxInfo_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created taxInfo").delete("/services/catalogs${new_taxInfo_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TaxInfo entity").exec(scn);
