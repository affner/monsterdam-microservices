...
public class CountryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all countries")
                    .get("/services/catalogs/api/countries")
                ...
                .exec(
                    http("Create new country")
                        .post("/services/catalogs/api/countries")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"name\": \"SAMPLE_TEXT\"" +
                                ", \"alpha2Code\": \"SAMPLE_TEXT\"" +
                                ", \"alpha3Code\": \"SAMPLE_TEXT\"" +
                                ", \"phoneCode\": \"SAMPLE_TEXT\"" +
                                ", \"thumbnailCountry\": null" +
                ...
                .repeat(5)
                .on(
                    exec(http("Get created country").get("/services/catalogs${new_country_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created country").delete("/services/catalogs${new_country_url}").headers(headers_http_authenticated))
                .pause(10)
        );
