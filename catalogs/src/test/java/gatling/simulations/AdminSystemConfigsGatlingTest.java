...
/**
 * Performance test for the AdminSystemConfigs entity.
 */
public class AdminSystemConfigsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all adminSystemConfigs")
                    .get("/services/catalogs/api/admin-system-configs")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new adminSystemConfigs")
                        .post("/services/catalogs/api/admin-system-configs")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_adminSystemConfigs_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created adminSystemConfigs")
                            .get("/services/catalogs${new_adminSystemConfigs_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created adminSystemConfigs")
                        .delete("/services/catalogs${new_adminSystemConfigs_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AdminSystemConfigs entity").exec(scn);
