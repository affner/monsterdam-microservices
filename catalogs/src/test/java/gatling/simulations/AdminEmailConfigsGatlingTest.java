...
/**
 * Performance test for the AdminEmailConfigs entity.
 */
public class AdminEmailConfigsGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all adminEmailConfigs")
                    .get("/services/catalogs/api/admin-email-configs")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new adminEmailConfigs")
                        .post("/services/catalogs/api/admin-email-configs")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_adminEmailConfigs_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created adminEmailConfigs")
                            .get("/services/catalogs${new_adminEmailConfigs_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created adminEmailConfigs")
                        .delete("/services/catalogs${new_adminEmailConfigs_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AdminEmailConfigs entity").exec(scn);
