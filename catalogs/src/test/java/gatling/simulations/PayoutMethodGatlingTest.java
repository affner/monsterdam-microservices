...
public class PayoutMethodGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all payoutMethods")
                    .get("/services/catalogs/api/payout-methods")
                ...
                .exec(
                    http("Create new payoutMethod")
                        .post("/services/catalogs/api/payout-methods")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created payoutMethod")
                            .get("/services/catalogs${new_payoutMethod_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created payoutMethod")
                        .delete("/services/catalogs${new_payoutMethod_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
