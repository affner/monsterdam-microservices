...
public class EmojiTypeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all emojiTypes")
                    .get("/services/catalogs/api/emoji-types")
                ...
                .exec(
                    http("Create new emojiType")
                        .post("/services/catalogs/api/emoji-types")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .repeat(5)
                .on(
                    exec(http("Get created emojiType").get("/services/catalogs${new_emojiType_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created emojiType").delete("/services/catalogs${new_emojiType_url}").headers(headers_http_authenticated))
                .pause(10)
        );
