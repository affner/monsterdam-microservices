 ...
 */
public class CurrencyGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all currencies")
                    .get("/services/catalogs/api/currencies")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new currency")
                        .post("/services/catalogs/api/currencies")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .repeat(5)
                .on(
                    exec(http("Get created currency").get("/services/catalogs${new_currency_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created currency").delete("/services/catalogs${new_currency_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Currency entity").exec(scn);
