...
public class GlobalEventGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all globalEvents")
                    .get("/services/catalogs/api/global-events")
                ...
                .exec(
                    http("Create new globalEvent")
                        .post("/services/catalogs/api/global-events")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created globalEvent").get("/services/catalogs${new_globalEvent_url}").headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created globalEvent")
                        .delete("/services/catalogs${new_globalEvent_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
