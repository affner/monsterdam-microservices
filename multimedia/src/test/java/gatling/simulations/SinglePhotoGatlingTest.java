...
public class SinglePhotoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all singlePhotos")
                    .get("/services/multimedia/api/single-photos")
                ...
                .exec(
                    http("Create new singlePhoto")
                        .post("/services/multimedia/api/single-photos")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created singlePhoto")
                            .get("/services/multimedia${new_singlePhoto_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created singlePhoto")
                        .delete("/services/multimedia${new_singlePhoto_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
