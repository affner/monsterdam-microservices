...
public class SingleAudioGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all singleAudios")
                    .get("/services/multimedia/api/single-audios")
                ...
                .exec(
                    http("Create new singleAudio")
                        .post("/services/multimedia/api/single-audios")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created singleAudio")
                            .get("/services/multimedia${new_singleAudio_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created singleAudio")
                        .delete("/services/multimedia${new_singleAudio_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
