...
public class ContentPackageGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all contentPackages")
                    .get("/services/multimedia/api/content-packages")
                ...
                .exec(
                    http("Create new contentPackage")
                        .post("/services/multimedia/api/content-packages")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created contentPackage")
                            .get("/services/multimedia${new_contentPackage_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created contentPackage")
                        .delete("/services/multimedia${new_contentPackage_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
