...
public class SingleVideoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all singleVideos")
                    .get("/services/multimedia/api/single-videos")
                ...
                .exec(
                    http("Create new singleVideo")
                        .post("/services/multimedia/api/single-videos")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created singleVideo")
                            .get("/services/multimedia${new_singleVideo_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created singleVideo")
                        .delete("/services/multimedia${new_singleVideo_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
