...
/**
 * Performance test for the UserTagRelation entity.
 */
public class UserTagRelationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all userTagRelations")
                    .get("/services/multimedia/api/user-tag-relations")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new userTagRelation")
                        .post("/services/multimedia/api/user-tag-relations")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                ...
                                ", \"isDeleted\": null" +
                                ", \"taggedUserId\": 0" +
                                ", \"tagStatus\": \"AUTHORIZED\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_userTagRelation_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created userTagRelation")
                            .get("/services/multimedia${new_userTagRelation_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created userTagRelation")
                        .delete("/services/multimedia${new_userTagRelation_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the UserTagRelation entity").exec(scn);
