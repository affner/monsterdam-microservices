...
public class SingleDocumentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all singleDocuments")
                    .get("/services/multimedia/api/single-documents")
                ...
                .exec(
                    http("Create new singleDocument")
                        .post("/services/multimedia/api/single-documents")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created singleDocument")
                            .get("/services/multimedia${new_singleDocument_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created singleDocument")
                        .delete("/services/multimedia${new_singleDocument_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
