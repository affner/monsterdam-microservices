...
public class VideoStoryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all videoStories")
                    .get("/services/multimedia/api/video-stories")
                ...
                .exec(
                    http("Create new videoStory")
                        .post("/services/multimedia/api/video-stories")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created videoStory").get("/services/multimedia${new_videoStory_url}").headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created videoStory")
                        .delete("/services/multimedia${new_videoStory_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
