...
public class SpecialRewardGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all specialRewards")
                    .get("/services/multimedia/api/special-rewards")
                ...
                .exec(
                    http("Create new specialReward")
                        .post("/services/multimedia/api/special-rewards")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created specialReward")
                            .get("/services/multimedia${new_specialReward_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created specialReward")
                        .delete("/services/multimedia${new_specialReward_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
